...
def _family_relationship(a, b):
    return In('str', 'family_relationship', a, b, "What is {1}'s relationship with {0}?")


# Is {0} married?
def _is_married(p):
    return In('bool', 'is_married', p, None, "Is {0} married?")


# What is the name of {0}'s spouse?
# Should always ask _is_married(p) before asking this
def _spouse_of(p):
    return In('str', 'spouse_of', p, None, "What is the name of {0}'s spouse?")
